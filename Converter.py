...
cal = Calverter()
df = pd.read_excel(r'C:\Users\Amgh\Documents\TseClient 2.0\IRX6XTPI0009.xlsx', usecols=[1])
...
persian = []
for date in all:
    jd = cal.gregorian_to_jd(int(date[0]), int(date[1]), int(date[2]))
    wday = cal.jwday(jd)
    persian.append(cal.jd_to_jalali(jd))

final = []
for persian_date in persian:
    final_val = str(persian_date[0]) + "-" + str(persian_date[1]) + "-" + str(persian_date[2])
    final.append(final_val)
df_final = DataFrame(final, columns=['date'])
df_final.to_excel("persian_date.xlsx")
